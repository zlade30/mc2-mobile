import { unregisterDevice } from "@/features/auth";
...
import { getErrorMessage, getFormattedDate } from "@/shared/lib/utils";
...
function ProfileContent() {
  ...
  const getCustomerRewardsQuery = useSuspenseQuery({
  ...
  const refetchProfile = useCallback(() => {
    void getCustomerRewardsQuery.refetch();
  }, [getCustomerRewardsQuery]);
  useRefetchOnAppFocus(refetchProfile);
  useRefreshOnTabPress(refetchProfile);

  const rewardsData = getCustomerRewardsQuery.data;
  const rewardsCount = rewardsData?.redeemable?.length ?? 0;

  const handleAppLockToggle = useCallback(
    async (value: boolean) => {
      if (value) {
        // Prefer biometrics/Face ID when available; only require PIN setup
        // upfront on devices without biometric unlock.
        const availableNow =
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          // onError already showed the error message modal
        }
      },
    });
  };
        ...
        <ScrollContent>
          ...
          <SurfaceCard $padding={Spacing.sm}>
            ...
            <ListRow
              ...
              left={<QuestionCircle size={22} color={screenIconColor} />}
              title="Help Center"
            ...
            />
            <ListRow
              onPress={() => router.push("/(customer)/terms-of-service")}
