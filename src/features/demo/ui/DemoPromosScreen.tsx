...
import { Modal, Platform, Pressable, TouchableOpacity, useWindowDimensions } from "react-native";
...
const ActivityCard = styled.View<{ $bg?: string }>`
...
export function DemoPromosScreen() {
  ...
  const filteredPromos = useMemo(() => {
    if (activeTab === "announcement") {
      return MOCK_PROMOS.filter((p: Promo) => p.type === "announcement");
    }
    return MOCK_PROMOS.filter(
      ...
      <Scroll showsVerticalScrollIndicator={false} onRefresh={async () => {}}>
        <ScrollContent>
          ...
          <TabBar>
            ...
            >
              <TabButtonText $active={activeTab === "promos"}>
                Promos
              </TabButtonText>
            </TabButton>
            <TabButton
          ...
          {!filteredPromos.length ? (
            ...
            filteredPromos.map((item) => {
              const goToDetail = () =>
                router.push(`/(demo)/customer/promo/${item.id}`);
