...
import { hideBottomModal, useBottomModalStore } from "./store";
...
export function BottomModalRoot() {
  ...
  const handleConfirmConfirm = async () => {
    ...
    // Default behavior: dismiss immediately, then run callback after hide animation.
    if (!keepOpenOnConfirm) {
      closeWithAnimation(callback as () => void);
      return;
    }

    // Loading behavior: keep the modal visible while `onConfirm` settles.
    setIsConfirmLoading(true);
    try {
      await Promise.resolve(callback());
      setIsConfirmLoading(false);
      // If `onConfirm` opened a new modal (e.g. called showMessage/showConfirm),
      // the store's `onConfirm` will no longer match the original callback.
      // In that case we must NOT close — doing so would kill the new modal.
      const nextState = useBottomModalStore.getState();
      if (nextState.onConfirm === callback) {
        closeWithAnimation();
      }
    } catch {
